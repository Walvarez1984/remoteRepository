...
*/
var jBox={
    imgs:['images/min.gif', 'images/close.gif', 'images/restaurar.gif','images/resize.gif'],
	boxes: [],minimizeorder: 0,
	init:function(id){
		var holder =$j('#jBoxHolder');
	    if (holder.length == 0){ 
		   holder = $j('<div id="jBoxHolder"></div>');
		   $j(document.body).append( holder );   
		}
		var box = $j('<div class="jBox" id="jb'+id+'"></div>' ).appendTo(holder);
			box.append('<div class="jBoxHandler">jBox<div class="jBoxControls"><img src="'+this.imgs[0]+'" title="Minimizar" />&nbsp;<img src="'+this.imgs[1]+'" title="Cerrar"/></div></div>')
            .append('<form name="formAux" style="margin:auto;"><div class="jBoxContent"></div></form>')
			
		    .append('<div class="jBoxStatus"><div class="jBoxResize" style="background: url('+this.imgs[3]+')  no-repeat;">&nbsp;</div></div');
		box.css( 'zIndex' , $j('div.jBox').length+99);
	...
	load:function(box,type,source,title,ajaxOpt){
	    ...
	    if (type=="inline"){
		     ...
		     box.content.html($j('#'+source).html());
	    }else if (type=="iframe"){
	...
	open:function( id, type, source, title,attr,ajaxOpt){	
		...
		if ($j('#jb'+id).length==0)
			...
			box=jBox.boxes[ parseInt( $j('#jb'+id).get(0).getAttribute('jBoxID') ) ];
		}
		...
		if( !box.minimizable  ){if (box.controls.find('img[@title=Minimizar]').length>0)box.controls.find('img[@title=Minimizar]').remove();}
	...
	setSize:function(box, w, h){box.css("width",Math.max(parseInt(w),50)+'px');box.content.css("height",Math.max(parseInt(h),50)+'px');},
	reSize:function(box, e){
	      box.css('width', Math.max(jBox.width+jBox.distancex, 50)+"px");
    ...
    setupControls:function(e){
	    ...
	    if (/Minimizar/i.test(sourceobj.getAttribute("title")))
		   j.minimize(sourceobj, box);
	    else if (/Restaurar/i.test(sourceobj.getAttribute("title")))
		   j.restore(sourceobj, box);
	    else if (/Cerrar/i.test(sourceobj.getAttribute("title")))
		   j.dispose(box);
	    return false;
	},
	cerrar:function(id){
	...
	showModel:function(){
      var model =$j('#jBox_hideIframe');      
 
	  if ( model.length==0 )
	  {
	     model = $j('<iframe id="jBox_hideIframe" scrolling="no" frameborder="0" style="position:absolute; top:0px; left:0px;-moz-opacity:0.5; opacity:0.5;filter:alpha(opacity=50);background:#000;"></iframe>');
		 model.appendTo( document.body );	     	     
		 $j(window).bind( 'resize', function(){jBox.showModel();})
	  }
	  jBox.getViewPoint();
    ...
    _retBox:function(dom){		
		return jBox.boxes[ parseInt(  $j(dom).parents('div.jBox').get(0).getAttribute('jBoxID') ) ];
	}
}
