/*
 * jquery.layout 1.3.0 - Release Candidate 29.1
 *
 * Copyright (c) 2010
...
$E.hide().data('autoHidden',true);};var setOuterHeight=function(el,outerHeight,autoHide){var $E=el;if(isStr(el))$E=$Ps[el];else if(!el.jquery)$E=$(el);h=cssH($E,outerHeight);$E.css({height:h,visibility:"visible"});if(h>0&&$E.innerWidth()>0){if(autoHide&&$E.data('autoHidden')){$E.show().data('autoHidden',false);if(!state.browser.mozilla)
$E.css(_c.hidden).css(_c.visible);}}
else if(autoHide&&!$E.data('autoHidden'))
...
classes+=(root+_slide+_hover)+(root+_pane+_slide+_hover);return $.trim(classes);};var addHover=function(evt,el){var e=el||this;$(e).addClass(getHoverClasses(e));};var removeHover=function(evt,el){var e=el||this;$(e).removeClass(getHoverClasses(e,true));};var onResizerEnter=function(evt){$('body').disableSelection();addHover(evt,this);};var onResizerLeave=function(evt,el){var
e=el||this,pane=$(e).data("layoutEdge"),name=pane+"ResizerLeave";timer.clear(name);if(!el){removeHover(evt,this);timer.set(name,function(){onResizerLeave(evt,e);},200);}
else if(!state[pane].isResizing)
$('body').enableSelection();};var _create=function(){initOptions();var o=options;if(false===_execCallback(null,o.onload))return false;if(!getPane('center').length){alert(lang.errCenterPaneMissing);return null;}
if(o.useStateCookie&&o.cookie.autoLoad)
loadCookie();state.browser={mozilla:$.browser.mozilla,webkit:$.browser.webkit||$.browser.safari,msie:$.browser.msie,isIE6:$.browser.msie&&$.browser.version==6,boxModel:$.support.boxModel};initContainer();initPanes();initResizable();sizeContent();if(o.scrollToBookmarkOnLoad)
with(self.location)if(hash)replace(hash);if(o.autoBindCustomButtons)initButtons();initHotkeys();if(o.resizeWithWindow&&!$Container.data("layoutRole"))
$(window).bind("resize."+sID,windowResize);$(window).bind("unload."+sID,unload);state.initialized=true;};var windowResize=function(){var delay=Number(options.resizeWithWindowDelay)||100;if(delay>0){timer.clear("winResize");timer.set("winResize",function(){timer.clear("winResize");timer.clear("winResizeRepeater");resizeAll();},delay);if(!timer.data["winResizeRepeater"])setWindowResizeRepeater();}};var setWindowResizeRepeater=function(){var delay=Number(options.resizeWithWindowMaxDelay);if(delay>0)
timer.set("winResizeRepeater",function(){setWindowResizeRepeater();resizeAll();},delay);};var unload=function(){var o=options;state.cookie=getState();if(o.useStateCookie&&o.cookie.autoSave)saveCookie();_execCallback(null,o.onunload);};var initContainer=function(){var
...
if(sel.substr(0,1)==="#")
return $Container.find(sel).eq(0);else{var $P=$Container.children(sel).eq(0);return $P.length?$P:$Container.children("form:first").children(sel).eq(0);}};var initPanes=function(){$.each(_c.allPanes.split(","),function(idx,pane){var
o=options[pane],s=state[pane],c=_c[pane],fx=s.fx,dir=c.dir,spacing=o.spacing_open||0,isCenter=(pane=="center"),CSS={},$P,$C,size,minSize,maxSize;$Cs[pane]=false;$P=$Ps[pane]=getPane(pane);if(!$P.length){$Ps[pane]=false;return true;}
...
s.noRoom=false;s.isVisible=true;if(!isCenter){s.isClosed=false;s.isSliding=false;s.isResizing=false;s.isHidden=false;}
...
_execCallback(pane,o.onresize_end||o.onresize);});if($Container.innerHeight()<2)
alert(lang.errContainerHeight.replace(/CONTAINER/,sC.ref));};var initHandles=function(panes){if(!panes||panes=="all")panes=_c.borderPanes;$.each(panes.split(","),function(i,pane){var $P=$Ps[pane];$Rs[pane]=false;$Ts[pane]=false;if(!$P)return;var
...
if(limit){ui.helper.addClass(helperLimitClass);window.defaultStatus="Panel has reached its "+
...
if(live)resizePanes(e,ui,pane);},stop:function(e,ui){$('body').enableSelection();window.defaultStatus="";$R.removeClass(resizerClass+" "+resizerPaneClass+" "+helperLimitClass);s.isResizing=false;_c.isLayoutBusy=false;resizePanes(e,ui,pane,true);}});var resizePanes=function(e,ui,pane,resizingDone){var
...
$Frames.each(function(){$("#"+$(this).data("layoutMaskID")).css($(this).position()).css({width:this.offsetWidth+"px",height:this.offsetHeight+"px"});});newSize=resizerPos-sC["inset"+c.side];manualSizePane(pane,newSize);}});};var destroy=function(){$(window).unbind("."+sID);$(document).unbind("."+sID);window[sID]=null;var
fullPage=(sC.tagName=="BODY"),root=o.paneClass,_open="-open",_sliding="-sliding",_closed="-closed",generic=[root,root+_open,root+_closed,root+_sliding],$P,pRoot,pClasses;$.each(_c.allPanes.split(","),function(i,pane){$P=$Ps[pane];if(!$P)return true;if(pane!="center"){if($Ts[pane])$Ts[pane].remove();$Rs[pane].remove();}
...
$Container.css($Container.data("layoutCSS"));if(fullPage)
...
if(!skipCallback&&(state.initialized||o.triggerEventsOnLoad)&&$P.is(":visible")){_execCallback(pane,o.onopen_end||o.onopen);if(s.isShowing)_execCallback(pane,o.onshow_end||o.onshow);if(state.initialized)_execCallback(pane,o.onresize_end||o.onresize);}};var slideOpen=function(evt_or_pane){var
...
open(pane,true);else
bindStopSlidingEvents(pane,true);};var slideClose=function(evt_or_pane){var
$E=(isStr(evt_or_pane)?$Ps[evt_or_pane]:$(this)),pane=$E.data("layoutEdge"),o=options[pane],s=state[pane],$P=$Ps[pane];if(s.isClosed||s.isResizing)
return;else if(o.slideTrigger_close=="click")
close_NOW();else if(o.preventQuickSlideClose&&_c.isLayoutBusy)
return;else
timer.set(pane+"_closeSlider",close_NOW,300);function close_NOW(e){if(s.isClosed)
bindStopSlidingEvents(pane,false);else
...
o=options[pane],z=_c.zIndex,$P=$Ps[pane],$R=$Rs[pane],trigger=o.slideTrigger_open;if(!$R||!o.slidable)return;if(trigger.match(/mouseover/))
trigger=o.slideTrigger_open="mouseenter";else if(!trigger.match(/click|dblclick|mouseenter/))
trigger=o.slideTrigger_open="click";$R.css("zIndex",!enable?z.pane_sliding:z.resizer_normal);$P.css("zIndex",!enable?z.pane_sliding:z.pane_normal);$R
...
o=options[pane],s=state[pane],trigger=o.slideTrigger_close,action=(enable?"bind":"unbind"),$P=$Ps[pane],$R=$Rs[pane];s.isSliding=enable;timer.clear(pane+"_closeSlider");if(enable)bindStartSlidingEvent(pane,false);if(!trigger.match(/click|mouseleave/))
...
_execCallback(pane,o.onresize_start);$P.css(_c[pane].sizeType.toLowerCase(),max(1,cssSize(pane,size)));s.size=size;$.extend(s,getElemDims($P));if($R&&$P.is(":visible"))$R.css(side,size+sC[inset]);sizeContent(pane);if(!skipCallback&&!skipResizeWhileDragging&&state.initialized&&s.isVisible){_execCallback(pane,o.onresize_end||o.onresize);if(o.resizeNestedLayout&&$P.data("layoutContainer"))
$P.layout().resizeAll();}
...
_execCallback(pane,o.onresize_start);$P.css(CSS);$.extend(s,getElemDims($P));if(s.noRoom)makePaneFit(pane);if(state.initialized)sizeContent(pane);}
...
if(!skipCallback&&state.initialized&&s.isVisible){_execCallback(pane,o.onresize_end||o.onresize);if(o.resizeNestedLayout&&$P.data("layoutContainer"))
$P.layout().resizeAll();}});};var resizeAll=function(){var
...
sizePane(pane,o.size,true,true);else{setSizeLimits(pane);makePaneFit(pane,false,true,true);}});sizeMidPanes("all",true,true);sizeHandles("all");o=options;$.each(_c.allPanes.split(","),function(i,pane){$P=$Ps[pane];if(!$P)return;if(state[pane].isVisible)
_execCallback(pane,o[pane].onresize_end||o[pane].onresize);if(o[pane].resizeNestedLayout&&$P.data("layoutContainer"))
$P.layout().resizeAll();});_execCallback(null,o.onresizeall_end||o.onresizeall);};var sizeContent=function(panes,remeasure){if(!panes||panes=="all")panes=_c.allPanes;$.each(panes.split(","),function(idx,pane){var
$P=$Ps[pane],$C=$Cs[pane],o=options[pane],s=state[pane],m=s.content;if(!$P||!$C||!$P.is(":visible"))return true;if(!_c.isLayoutBusy||m.top==undefined||remeasure||o.resizeContentWhileDragging){_measure();if(m.hiddenFooters>0&&$P.css("overflow")=="hidden"){$P.css("overflow","visible");_measure();$P.css("overflow","hidden");}}
var newH=s.innerHeight-(m.spaceAbove-s.css.paddingTop)-(m.spaceBelow-s.css.paddingBottom);if(!$C.is(":visible")||m.height!=newH){setOuterHeight($C,newH,true);m.height=newH;};function _below($E){return max(s.css.paddingBottom,(parseInt($E.css("marginBottom"))||0));};function _measure(){var
